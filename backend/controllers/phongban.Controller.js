...
export const addPhongBan = async (req, res) => {
    ...
    try {
        ...
        if (existing.length > 0) {
            return res.status(409).json({ message: `Phòng ban "${tenPBTrimmed}" đã tồn tại.` });
        }
...
export const editPhongBan = async (req, res) => {
    ...
    try {
        ...
        if (duplicate.length > 0) {
            return res.status(409).json({ message: `Tên "${tenPBTrimmed}" đã được dùng bởi phòng ban khác.` });
        }
