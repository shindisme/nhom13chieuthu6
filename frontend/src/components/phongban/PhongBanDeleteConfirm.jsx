...
function PhongBanDeleteConfirm({ record, onClose, onDeleted }) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    try {
      setLoading(true);
      await phongBanService.delete(record.MaPB || record.id);
      toast.success("Đã xóa phòng ban thành công!");
      onDeleted();
    } catch (err) {
      toast.error(err?.response?.data?.message || "Xóa thất bại! Phòng ban này có thể đang có nhân viên.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 text-center">
        ...
        <h3 className="font-bold text-slate-800 text-base">Xóa phòng ban?</h3>
        <p className="text-sm text-slate-500 mt-2">
          Bạn có chắc chắn muốn xóa phòng ban <span className="font-semibold text-slate-700">{record.TenPB}</span> không?
        </p>
...
export default PhongBanDeleteConfirm;
